...
export const FraudCheck: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">Real-time Fraud Analysis</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-fit">
          <h2 className="text-lg font-bold text-slate-900 mb-4 border-b pb-2">Transaction Details</h2>
        ...
        <div className="space-y-6">
          {!result && !loading && (
            <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-12 text-center h-full flex flex-col items-center justify-center text-slate-400">
          ...
          {result && (
            <>
              ...
              <div className={`
                ...
                ${result.isFraud ? 'bg-red-50 border-red-500' : 'bg-green-50 border-green-500'}
              `}>
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className={`text-xl font-bold ${result.isFraud ? 'text-red-700' : 'text-green-700'}`}>
                      {result.isFraud ? 'High Fraud Probability' : 'Low Fraud Risk'}
                    </h3>
                    <p className={`text-sm mt-1 ${result.isFraud ? 'text-red-600' : 'text-green-600'}`}>
                      Confidence Score: <span className="font-bold text-lg">{(result.confidence * 100).toFixed(1)}%</span>
                    </p>
                  </div>
                  <div className={`p-3 rounded-full ${result.isFraud ? 'bg-red-200' : 'bg-green-200'}`}>
                    {result.isFraud ? (
                      <ShieldAlert className={`w-8 h-8 ${result.isFraud ? 'text-red-600' : 'text-green-600'}`} />
                    ) : (
                      <ShieldCheck className={`w-8 h-8 ${result.isFraud ? 'text-red-600' : 'text-green-600'}`} />
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-lg font-bold text-slate-900">Model Explanation (SHAP-like)</h3>
                </div>
                ...
                <div className="mt-6 pt-6 border-t border-slate-100">
                   ...
                   <div className="space-y-2">
                     <div className="flex items-center text-sm">
                       <span className="w-24 text-slate-500">Amount</span>
                     ...
                     <div className="flex items-center text-sm">
                       <span className="w-24 text-slate-500">Merchant</span>
                       <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden ml-2">
